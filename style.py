...
def main():
    ...
    if os.path.exists(options.checkpoint_dir):
        print('Directory %s already exists!' % options.checkpoint_dir)
    else:
        print('Directory %s does not exist! Creating it...' % options.checkpoint_dir)
    ...
    for preds, losses, i, epoch in optimize(*args, **kwargs):
        ...
        # to_print = (style_loss, content_loss)
        print('style: %s, content:%s, tv: %s' % to_print)
        # print('style: %s, content:%s, tv: 0' % to_print)
        if options.test:
    ...
    elapsedTime = datetime.timedelta(seconds=elapsedTime)
